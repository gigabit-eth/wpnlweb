	...
	public function settings_page() {
		...
		<div class="wrap">
			...
			<h1>
				...
				if ( ! $css_exists ) :
					?>
					<span style="color: #dc3545; font-size: 16px; font-weight: normal;"> - Missing Styles</span><?php endif; ?>
			</h1>
			...
			<div class="wpnlweb-admin-wrapper" 
			<?php
			if ( ! $css_exists ) :
				?>
				style="border: 2px solid #dc3545; background: #fff5f5;" <?php endif; ?>>
				<div class="wpnlweb-admin-header" 
				<?php
				if ( ! $css_exists ) :
					?>
					style="background: #fee; padding: 20px; border-bottom: 1px solid #fcc;" <?php endif; ?>>
					<p class="wpnlweb-admin-subtitle" 
					<?php
					if ( ! $css_exists ) :
						?>
						style="color: #666; margin: 5px 0 0 0;" <?php endif; ?>>
...
.wpnlweb-search-button { background: var(--wpnlweb-primary-color); }</code></pre>
	...
	private function generate_admin_preview( $theme_mode, $primary_color, $custom_css ) {
		...
		<style>
			...
			if ( ! empty( $custom_css ) ) :
				?>
				<?php echo esc_html( wp_strip_all_tags( $custom_css ) ); ?><?php endif; ?>
		</style>
